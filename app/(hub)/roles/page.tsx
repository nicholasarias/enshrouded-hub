...
// --- Top Bar ---
function TopBar(props: { discordName: string | null; level: number }) {
    ...
    >
      ...
      <style>{`
        @keyframes driftUp {
          ...
          50% {
            transform: translateY(15%) scale(1.5) rotate(3deg);
          }
          90% {
            opacity: 0.1;
          }
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {/* IMPORTANT: clamp to 2 lines so all cards stay uniform */}
        <div
          style={{
            fontSize: "0.78rem",
            color: THEME.textAsh,
...
export default function RolesPage() {
  ...
  async function loadAll() {
    ...
    try {
      const [metaRes, selectedRes, officerRes, sessionRes] = await Promise.all([
        fetch(`/api/guild/role-meta?guildId=${GUILD_ID}`, { cache: "no-store" }),
        fetch(`/api/me/selected-roles?guildId=${GUILD_ID}`, { cache: "no-store" }),
        fetch(`/api/me/is-officer?guildId=${GUILD_ID}`, { cache: "no-store" }),
  ...
  async function selectRole(roleId: string) {
    ...
    try {
      const res = await fetch("/api/me/select-role", {
        ...
        body: JSON.stringify({ guildId: GUILD_ID, roleId }),
      });
  ...
  async function officerReset(discordUserId: string) {
    ...
    try {
      const res = await fetch("/api/officer/reset-user-roles", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ guildId: GUILD_ID, discordUserId: did }),
      });
      const json = await res.json().catch(() => null);
  ...
  async function officerSet(discordUserId: string, roleId: string) {
    ...
    try {
      const res = await fetch("/api/officer/set-user-role", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ guildId: GUILD_ID, discordUserId: did, roleId }),
      });
      const json = await res.json().catch(() => null);
  ...
  return (
    ...
    >
      <TopBar discordName={discordName} level={currentLevel} />
      <ShroudFog />
            ...
            >
              {!combatSelected ? (
                ...
                >
                  ...
                  {/* Fake Peasant card: same cell rules, same height, looks clickable */}
                  <div
                    key="__peasant__"
                    style={{
                      position: "relative",
